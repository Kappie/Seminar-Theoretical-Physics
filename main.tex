...
First we group terms in the sum over $k$ into $q$ sums over $m$. We haven't assumed anything new about any values.

This is just rewritting.
\begin{align*}
H &= \sum_{0 \leq k < \frac{2\pi}{a}} (\frac{1}{2}\epsilon_k c_k^{\dagger}c_k + \Delta_\phi c_k^{\dagger}c_{k+Q} + \text{H.c.})\\
...
The antisymmetry of the Chern numbers is easily seen in the antisymmetry of the colouring of the spectrum as a function of $\frac{p}{q}$ ie the Hofstadter Butterfly figure \textbf{???}.\\
...
The CDW spectrum is a Hofstadter butterfly. It turns out that the CDW is very similar to that where the butterfly originally arose.
